...
- 单页 QA：初检通过；单一种子、未萌发阶段、D01 左/D02 右和文字区符合锁定。
- 单页状态：无字图已生成并标准化为 `1536×2048`，待排版
...
- 单页 QA：初检通过；渗水路径、完整种子、D01 左/D02 右、无脸雨点和文字区符合锁定。
- 单页状态：无字图已生成并标准化为 `1536×2048`，待排版
...
- 单页 QA：初检通过；种皮裂缝、无根芽、单一种子和左上文字区符合锁定。
- 单页状态：无字样张已生成，待统一尺寸与排版
...
- 单页 QA：第二次定向生成后通过；根短且向下，无绿芽、无分叉，阶段与物理接触符合锁定。
- 单页状态：无字图已生成并标准化为 `1536×2048`，待排版
...
- 单页 QA：初检通过；单芽、芽钩、出土接触、D01 左/D02 右和文字区符合锁定。
- 单页状态：无字样张已生成，待统一尺寸与排版
...
- 单页 QA：初检通过；恰好两片光滑子叶、无真叶、无太阳脸，文字区符合锁定。
- 单页状态：无字图已生成并标准化为 `1536×2048`，待排版
...
- 单页 QA：初检通过；下方子叶与上方真叶差异清楚，单一主茎和 D01/D02 符合锁定。
- 单页状态：无字图已生成并标准化为 `1536×2048`，待排版
...
- 单页 QA：第二次定向生成后通过；采用纯地上视图，单一闭合花苞、株高、D01/D02 和右上安全区符合锁定。
- 单页状态：无字图已生成并标准化为 `1536×2048`，待排版
...
- 单页 QA：第二次定向生成后通过；采用纯地上视图，单花盘、物种、D01 左/D02 右、色彩和左上文字区符合锁定。
- 单页状态：无字样张已生成，待统一尺寸与排版
